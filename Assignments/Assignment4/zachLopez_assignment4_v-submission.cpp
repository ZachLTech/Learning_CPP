 ...
 * 
 * - You'll notice that the 2 classes have an unused constructor that I added just in case it wants to be seen but the only difference is it takes every member variable as a parameter. The one I actually used for the example though is the one which includes only necessary variables for parameters as my logic functions calculate the rest.
 * 
...
class Spring24 : public COP3014 {
    ...
    public:
        ...
        // Setter and Getter for labGrade since rubric calls for getters and setters on all member variables
...
// Spring Constructors Like taught in lab
...
void COP3014::output_COP3014(bool base){
    ...
    if (midtermTaken == false){
        cout << "\tMidterm: 0.00 - NOT TAKEN" << endl;
    }
    else{
        cout << "\tMidterm: " << grades[4] << endl;
    }
    if (finalTaken == false){
        cout << "\tFinal: 0.00 - NOT TAKEN" << endl;
    }
    else{
...
// Sorry if this was longer than some other students with extra comments, I just wanted to make everything work and put some of my thought process via notes throughout my code :)